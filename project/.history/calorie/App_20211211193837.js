...
function SearchScreen({ navigation }) {
  ...
  const onSubmit = e => {
    setTimeout(() => { let expression= e.nativeEvent.text.toString()
    const options = {
      method: 'GET',
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TextInput  onChange={onSubmit} placeholder={'search for food'} style={{ color: 'cyan', fontSize: 32, backgroundColor:'grey', borderRadius:10   }}/>
      <View >
      <FlatList
        data={results}
        renderItem={({ item }) => {
