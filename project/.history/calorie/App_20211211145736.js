...
function SearchScreen({ navigation }) {
  ...
  function onSubmit (expression) {
    ...
    axios.request(options).then(function (response) {
      setEnergy(response.data.items[0].calories)
      console.log(energy)
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', }}>
      <TextInput  onSubmitEditing={onSubmit(value)} placeholder={'search for food'} style={{ color: 'cyan', fontSize: 32, backgroundColor:'grey',  width:'90%',borderRadius:10   }}/>
      <Text style={[,{color:'black'}]}>df{energy}</Text>
