...
function DetailsScreen({navigation,route}){
  ...
  const [text, setText] = useState('');
    ...
    let finalweight = selectedweight*text.toInt()
    ...
    return(
      <SafeAreaView style={styles.container}>
         ...
         <View style={[,{margin:5,display:'flex',flexDirection:'row',justifyContent:'space-evenly',alignItems:'center'}]}>
          <TextInput placeholderTextColor={'black'}  keyboardType='numeric' textAlign={'center'} style={[,{backgroundColor:'#6dbc4e',borderBottomWidth:1, color:'white',height:40, width:70}]} onChangeText={text => setText(text)}>{serving}</TextInput>
