...
function WeightProgressScreen({ navigation, route }) {
  ...
  const getMultiple = async () => {
    ...
    try {
      const values = await AsyncStorage.multiGet([yesterday.getTime().toString(),yesterday1.toString(),yesterday2.toString(),yesterday3.toString(),yesterday4.toString(),yesterday5.toString()])
...
for(let i=0; i<str.length;i++) {
    if (str[i] === "w") indices.push(i);
}
let resu=[]
for(let i=0;i<indices.length;i++){
  for(let j=indices[i];j<=indices[i]+12;j++){
  resu[i]+=str[j]
  }
}
console.log(resu[0].substr(10,))
