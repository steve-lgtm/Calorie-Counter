...
totalEnergy=totalEnergy+parseInt((diaryyy.entries[i].energy/100)*diaryyy.entries[i].finalweight)}
  ...
  weight=parseInt(diaryyy.entries[i].weight)
    }
}
...
const FiberData = [
 ...
 {day}, {diaryKeyDate.toString().substr(8, 2)}. {(diaryKeyDate.getMonth())+1}.</Text>
      ...
      <TouchableOpacity onPress={() => setModalVisible(true)}>
       ...
       <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}><Text style={{paddingLeft:5,fontSize:29,  color:'white'}}>{weight} kg</Text><MaterialCommunityIcons style={{}} name="plus-circle" color={"green"} size={20} />
</View>
        </View>
        </TouchableOpacity>
        <TouchableOpacity onPress={()=>navigation.navigate('Search')}>
