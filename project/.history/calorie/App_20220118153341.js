...
function WeightProgressScreen({ navigation, route }) {
  ...
  yesterdays.setDate(yesterdays.getDate()-5)

  const getMultiple = async () => {

    try {
      const values = await AsyncStorage.multiGet([  (yesterday.setDate(yesterdays.getDate()-5)).getTime().toString(),yesterdays.getTime().toString()])
