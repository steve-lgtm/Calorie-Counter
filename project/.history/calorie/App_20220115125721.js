...
function OverviewTabScreen({ navigation }) {
  ...
  const [date, setDate] = useState(diaryKeyDate);
  const [show, setShow] = useState(false);
console.log(length(diaryyy.entries))
  for(let i=0;i<length(diaryyy.entries);i++){

  }

  const totalEnergy = diaryyy.entries.reduce((prevValue, currentValue) => prevValue + Number((currentValue.energy/100)*currentValue.finalweight),0,);
