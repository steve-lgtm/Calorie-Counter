...
function DetailsScreen({navigation,route}){
    ...
    return(
      <SafeAreaView style={styles.container}>
         ...
         <Text style={[,{margin:10,fontSize:16}]}>Množstvo</Text>
         <View style={[,{margin:5,display:'flex',flexDirection:'row',justifyContent:'space-evenly',alignItems:'center'}]}>
          <TextInput style={[,{backgroundColor:'green',}]} placeholderTextColor={'black'} keyboardType='numeric' textAlign={'center'} style={[,{backgroundColor:'white',height:40, width:70}]}></TextInput>
          <Text style={[,{fontSize:15}]}>X</Text>
          <TextInput style={[,{backgroundColor:'white',height:40, width:230}]}></TextInput>
...
function SearchScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
     <StatusBar barStyle="dark-content" backgroundColor="#fdca40" />
      <TextInput textAlign={'center'}autoFocus onSubmitEditing={onSubmit} placeholderTextColor={'white'} placeholder={'Search for food'} style={{ marginTop:1,marginBottom:5,color: 'black', fontSize: 16, backgroundColor:'grey', borderTopWidth:0.5, borderBottomWidth:0.2 }}/>
      <View >
      <FlatList
        data={results}
        renderItem={({ item }) => {
