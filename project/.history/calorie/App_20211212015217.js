  ...
  return (
    <SafeAreaView style={styles.container}>
     <StatusBar barStyle="dark-content" backgroundColor="#fdca40" />
      ...
      <Button title={'gotodetails'} onPress={() => navigation.navigate('Details')}/>
      <View >
      <FlatList
        data={results}
        renderItem={({ item }) => {
...
export default function App({navigation}) {
  const { items, ...rest } = this.props;
    const filteredItems = items.filter(item => item.key !== "Details");
  return (
    <SafeAreaProvider>
      ...
      <Drawer.Navigator initialRouteName="Home" drawerContent={props => {
          const filteredProps = {
            ...
            state: {
              ...
              routes: props.state.routes.filter(route => route.name !== 'Details'),
            },
          };
          return (
        ...
        >
        <Drawer.Screen drawerLabel={null} name='Details' component={DetailsScreen}/>
        ...
        <Drawer.Screen  options={{headerStyle: {
            ...
            },headerTintColor: 'black',}} name="Search" component={SearchScreen} />
