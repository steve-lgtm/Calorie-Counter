...
function UpdateFoodScreen({ navigation, route }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <StatusBar barStyle="dark-content" backgroundColor="#fdca40" />
      <View style={[, { display: 'flex', alignItems: 'center', width:400, borderRadius: 10, backgroundColor: 'black', borderBottomWidth: 1,borderColor:'white', marginVertical: 10, marginHorizontal: 5, padding: 2 }]}>
        <Text style={[, { paddding:10, fontSize: 35, color:'white'  }]}>{name}</Text>
      </View>
      <ScrollView>
      <View style={[, { display: 'flex', borderRadius: 10,width:400, backgroundColor: '#6dbc4e', marginVertical: 10, marginHorizontal: 5, padding: 2 }]}>
        <Text style={[, { color: 'white', margin: 10, fontSize: 16 }]}>{t("Quantity")}</Text>
