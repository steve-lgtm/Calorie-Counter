...
function DetailsScreenBar({ navigation, route }) {
  ...
  const food = route.params.food.product
  const energy = food.nutriments['energy-kcal_100g'].toFixed()
  const name = food.product_name
  const quantity = food.product_quantity
  const energykj = food.nutriments['energy-kj_100g'].toFixed()
  const protein = food.proteins_100g
  const catbohydrates = food.carbohydrates_100g
  const fat = food.fat_100g
  const sugar = food.sugars_100g
  const serving = food.serving_quantity
  ...
  return (
    <SafeAreaView style={styles.container}>
      <StatusBar barStyle="dark-content" backgroundColor="#fdca40" />
      <View style={[, { display: 'flex', alignItems: 'center', width:200, borderRadius: 10, backgroundColor: '#6dbc4e', marginVertical: 10, marginHorizontal: 5, padding: 2 }]}>
        <Text style={[, { fontSize: 35 }]}>{name}</Text>
      </View>
      <View style={[, { display: 'flex', borderRadius: 10, backgroundColor: '#6dbc4e', marginVertical: 10, marginHorizontal: 5, padding: 2 }]}>
        <Text style={[, { color: 'white', margin: 10, fontSize: 16 }]}>Množstvo</Text>
      ...
      <View >
        {item == true ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const onFoodItemPress = e => {
                navigation.navigate('Detail potraviny barcode', {
                  food: item,
                });
              };
      ...
      <SafeAreaProvider>
        ...
        <NavigationContainer>
          <Drawer.Navigator drawerContent={props => <DrawerContent {...props} />}>
            ...
            <Drawer.Screen name="Detail potraviny barcode" component={DetailsScreenBar} options={{ headerStyle: { backgroundColor: '#fdca40', }, headerTintColor: 'black', }} />
