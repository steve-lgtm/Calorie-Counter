...
function WeightProgressScreen({ navigation, route }) {
  ...
  let yesterdays = new Date(today)
  yesterdays.setDate(yesterdays.getDate()-2)

  const getMultiple = async () => {

    try {
      const values = await AsyncStorage.multiGet([yesterday.getTime().toString(),yesterdays.getTime().toString()])
      const test= JSON.stringify(values[0])
      console.log(typeof(test),test.substring(20,))
      setResponses(test)
    } catch(e) {
      // read error
    }
