...
function SearchScreen({ navigation, route }) {
  ...
  const onSubmit1 = e => {
   ...
   axios.request(options).then(function (response) {
     setResults(response.data.product.nutriments)
     console.log(response.data.product.nutriments)
 ...
 console.log('eee')
  ...
  return (
    <SafeAreaView style={styles.container}>
     <StatusBar barStyle="dark-content" backgroundColor="#fdca40" />
      {route.params==undefined ? ( <TextInput textAlign={'center'} autoFocus={true} onSubmitEditing={onSubmit} placeholderTextColor={'white'} placeholder={'Search for food'} style={{ marginTop:1,marginBottom:5,color: 'black', fontSize: 16, backgroundColor:'grey', borderTopWidth:0.5, borderBottomWidth:0.2 }}/>) : 
            <TextInput textAlign={'center'} autoFocus={true} onSubmitEditing={onSubmit1} placeholderTextColor={'white'} placeholder={'Search for food'} style={{ marginTop:1,marginBottom:5,color: 'black', fontSize: 16, backgroundColor:'grey', borderTopWidth:0.5, borderBottomWidth:0.2 }}>{route.params.barcode}</TextInput>
          }
      <View >
      {route.params==undefined ? (
      <FlatList
      ...
      <FlatList
        ...
        renderItem={({code}) => {
          ...
          return <FoodItemBar food={code} onPress={onFoodItemPress}/>;
        }}
      />)}
    </View>
    </SafeAreaView>
