...
function OverviewTabScreen({ navigation }) {
 ...
 {day}, {diaryKeyDate.toString().substr(8, 2)}. {(diaryKeyDate.getMonth())+1}.</Text>
      ...
      <View style={{display: 'flex',marginTop:15,height:90,width:420,justifyContent:'space-evenly',flexDirection:'row'}}>
       ...
       <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}><Text style={{paddingLeft:5,fontSize:30,  color:'white'}}>{total} kcal</Text><MaterialCommunityIcons style={{}} name="plus-circle" color={"green"} size={20} />
</View>
        </View>
        </TouchableOpacity>
        <TouchableOpacity onPress={()=>navigation.navigate('Search')}>
      ...
      <TouchableOpacity onPress={()=>navigation.navigate('Search')}>
       ...
       strokeColor={'white'}
                ...
                return <Text style={{fontSize: 30, color:'black'}}>{(100*(total/goalEnergy)).toFixed()} %</Text>;
                }}
            />
